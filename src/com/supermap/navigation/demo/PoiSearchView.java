...
public class PoiSearchView extends BaseView{
	...
	static private PoiSearchView mNearbyView = null;
...
//	private Recordset mRecordsetResult = null;
	...
	static public PoiSearchView getInstance(Context context, MapView mapView, View mainView) {
		if (mNearbyView == null) {
			mNearbyView = new PoiSearchView(context, mapView, mainView);
		}
		return mNearbyView;
	}
	
	private PoiSearchView(Context context, MapView mapView, View mainView) {
		mContext = context;
		mMainView = mainView;
		mMapView = mapView;
	...
	private void initView() {	
		...
		mViewMainView= LayoutInflater.from(mContext).inflate(R.layout.poi_search_view, null);
		mViewMainView.setClickable(true);
		
		edtSearch = (EditText) mViewMainView.findViewById(R.id.edt_poi_search_view_back);

		btnSearch = (Button) mViewMainView.findViewById(R.id.btn_poi_search_view_search);
		btnSearch.setOnClickListener(buttonOnClickListener);

		btnBack = (ImageButton) mViewMainView.findViewById(R.id.btn_poi_search_view_back);
		btnBack.setOnClickListener(buttonOnClickListener);
		
		lstSearchResult = (ListView) mViewMainView.findViewById(R.id.lst_poi_search_view_result);
	...
	OnClickListener buttonOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			switch (v.getId()) {	
			case R.id.btn_poi_search_view_search:	
				String name = edtSearch.getText().toString();
				searchByname(name, -1);
				hideSoftInput(edtSearch);
				break;

			case R.id.btn_poi_search_view_back:
...
//				mPoiSearchResultAdapter.notifyDataSetInvalidated();
				break;
	...
	private void showPoint(int itemPosition) {
		Point2D pt = vtPoint.get(itemPosition);
		
		showPointByCallOut(pt, "poiPoint", R.drawable.icon_poi_mark, CalloutAlignment.BOTTOM);
		GeoToolkit.MapToLongitude_Latitude(mMapControl.getMap(), pt);

		NaviManager.getInstance(mContext).setPoiPoint(pt.getX(), pt.getY());
		
		PoiShowView poiShowView = PoiShowView.getInstance(mContext, mMapView, mMainView);
		// 显示POI展示
		if (!poiShowView.isShowing()) {
			ViewManager.getInstance().addView(poiShowView);
		}
		
		int id = vtID.get(itemPosition);
		poiShowView.setPoiID(id);
		
		String name = vtName.get(itemPosition);
		poiShowView.setPoiName(name);
		poiShowView.show();	
		
//		mMapControl.getMap().setCenter(pt);
		mMapControl.panTo(pt, 100);
		mMapControl.zoomTo(1/4999.0, 100);
//		mMapControl.getMap().setScale(1/4999.0);
